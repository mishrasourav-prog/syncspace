const PENDING_REGISTRATION_EMAIL_KEY = "syncspace:pending-registration-email";

export function savePendingRegistrationEmail(email: string): void {
...
export function readPendingRegistrationEmail(): string | null {
  const email = window.sessionStorage.getItem(PENDING_REGISTRATION_EMAIL_KEY);
