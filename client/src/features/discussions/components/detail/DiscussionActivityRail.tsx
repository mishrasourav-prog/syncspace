...
const SUPPORTED_DISCUSSION_ACTIVITY = new Set([
  "discussion.created",
  "discussion.updated",
  "discussion.pinned",
  "discussion.unpinned",
...
export function DiscussionActivityRail({ projectId, discussionId }: DiscussionActivityRailProps) {
  ...
  const discussionActivities = (activitiesQuery.data ?? [])
    .filter((activity) => {
      if (!SUPPORTED_DISCUSSION_ACTIVITY.has(activity.action)) return false;
      if (activity.entityType === "discussion") {
        return activity.entityId === discussionId;
      }
      if (activity.entityType === "discussion_reply") {
        return activity.metadata?.discussionId === discussionId;
      }
      return false;
    })
    .slice(0, 8);
