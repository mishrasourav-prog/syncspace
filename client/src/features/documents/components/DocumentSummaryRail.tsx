import { FileText, PenSquare, UserRound } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3 text-sm">
          <span className="text-muted">Loaded total</span>
        ...
        <CountRow
          ...
          count={archivedLoadedCount}
          hasMore={archivedHasMore}
          loading={archivedLoading}
          unavailable={archivedUnavailable}
        />

        <div className="flex items-center justify-between gap-3 text-sm">
          <span className="text-muted">Updated in last 7 days</span>
        ...
        <div className="mt-4 border-t border-border pt-3">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-muted">
            <span className="flex items-center gap-1">
              ...
              {mostRecentlyUpdated.updatedBy?.name ?? "Unavailable member"}
            </span>
            ...
            <span>{formatRelativeTime(mostRecentlyUpdated.updatedAt)}</span>
          </div>
        </div>
      )}
      ...
      {(activeUnavailable || archivedUnavailable) && (
        <p className="mt-3 text-[11px] text-warning">
          Some document counts are unavailable. Loaded data is shown where
          possible.
        </p>
      )}

      <p className="mt-4 text-[11px] text-muted/70">
        Counts and summaries reflect only loaded documents because the API does
        not provide exact totals.
      </p>
    </section>
  );
}
