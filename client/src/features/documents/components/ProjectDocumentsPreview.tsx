...
export function ProjectDocumentsPreview({ projectId, search }: ProjectDocumentsPreviewProps) {
  ...
  const visibleDocuments = showAll ? documents : documents.slice(0, INITIAL_VISIBLE);
    ...
    >
      ...
      {!documentsQuery.isLoading && !documentsQuery.isError && documents.length === 0 && (
        <div className="rounded-xl border border-dashed border-border bg-background/40 px-4 py-8 text-center">
          ...
          <p className="mt-2 text-body">{search ? "No documents match your search." : "No documents yet."}</p>
        </div>
      )}
      ...
      {!documentsQuery.isLoading && !documentsQuery.isError && documents.length > 0 && (
        <div className="space-y-2">
          {visibleDocuments.map((document) => (
            <div key={document._id} className="flex items-center gap-3 rounded-lg border border-border/60 px-3 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-foreground">{document.title || "Untitled document"}</p>
                <p className="truncate text-caption">
                  {document.createdBy?.name ?? "Unknown"}
                  {document.updatedBy && document.updatedBy._id !== document.createdBy?._id
          ...
          {nextCursor && (
            <p className="pt-1 text-center text-[11px] text-muted/70">Showing the 50 most recent documents.</p>
          )}
        </div>
      )}
    </section>
