  ...
  return (
    <section className="rounded-xl border border-border bg-surface/60 p-4 shadow-soft">
      ...
      <p className="mb-3 text-[11px] text-muted/80">
        Project members can edit this document. Concurrent saves are protected by revision checks.
      </p>

      {isLoading && (
        <div className="flex -space-x-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <Skeleton key={index} className="h-8 w-8 rounded-full ring-2 ring-surface" />
          ))}
        </div>
      )}
      ...
      {isError && (
        <div className="flex items-center gap-2 text-sm text-muted">
          ...
          <button type="button" onClick={onRetry} className="font-medium text-primary hover:text-primary/80">
            Retry
          </button>
        </div>
      )}
      ...
      {!isLoading && !isError && (
        <>
          <div className="flex -space-x-2">
            {visible.map((member) => (
              <div key={member._id} className="relative" title={`${member.user.name} · ${member.role}`}>
                <Avatar src={member.user.avatar} name={member.user.name} size="sm" className="ring-2 ring-surface" />
                {member.role === "admin" && (
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
            ...
            {canInvite && (
              <button type="button" onClick={onInvite} className="font-medium text-primary hover:text-primary/80">
