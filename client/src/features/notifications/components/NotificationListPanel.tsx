...
import type { NotificationFilter, NotificationItem, NotificationSort } from "../notification.types";
...
export function NotificationListPanel({
  ...
  return (
    <section aria-label="Notification list" className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-surface/60 shadow-soft">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-4 py-3">
        <div>
          <h2 className="text-h3 text-foreground">{FILTER_LABELS[filter]}</h2>
          <p className="text-caption" aria-live="polite">
            {visibleNotifications.length} {visibleNotifications.length === 1 ? "notification" : "notifications"} shown
          </p>
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger className="h-8 w-auto gap-1.5 px-2.5 text-xs" aria-label="Sort notifications">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {visibleNotifications.length === 0 ? (
          <NotificationEmptyState filter={filter} hasSearchQuery={hasSearchQuery} onClear={onClearSearchOrFilter} />
        ) : (
          <ul role="list">
      ...
      {totalLoadedCount > 0 && (
        <div className="shrink-0 border-t border-border px-4 py-2.5 text-center text-caption">
          {totalLoadedCount >= 50 ? (
            <span>
              Showing your latest 50 notifications. Older notifications are not available through the current API.
            </span>
          ) : (
