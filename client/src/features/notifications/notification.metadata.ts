...
export function getTaskAssignedMetadata(
  ...
  return {
    ...
    taskType:
      taskType === "task" || taskType === "issue" ? taskType : undefined,
  };
}

export function getTaskStatusChangedMetadata(
  notification: NotificationItem,
): TaskStatusChangedMetadata {
  if (!isRecord(notification.metadata)) return {};
...
export function getDiscussionReplyMetadata(
  notification: NotificationItem,
): DiscussionReplyMetadata {
  if (!isRecord(notification.metadata)) return {};
...
export function getNotificationSupplementalText(
  notification: NotificationItem,
): string | undefined {
  switch (notification.type) {
    ...
    case "task_status_changed": {
      const { previousStatus, currentStatus } =
        getTaskStatusChangedMetadata(notification);
    ...
    case "workspace.member_joined": {
      const { workspaceName, role } =
        getCollaborationNotificationMetadata(notification);
