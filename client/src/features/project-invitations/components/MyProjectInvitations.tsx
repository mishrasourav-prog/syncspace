...
export function MyProjectInvitations() {
  ...
  function accept(invitation: ProjectInvitation) {
    acceptMutation.mutate(invitation, {
      onSuccess: () => toast.success(`You joined ${invitation.projectName ?? "the project"}.`),
      onError: (error) => toast.error(error.message ?? "Unable to accept project invitation."),
    });
  }
  ...
  function reject(invitation: ProjectInvitation) {
    rejectMutation.mutate(invitation, {
      ...
      onError: (error) => toast.error(error.message ?? "Unable to decline project invitation."),
    });
  }

  return (
    <section id="project-invitations" aria-labelledby="project-invitations-heading">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <FolderKanban className="h-4 w-4 text-primary" />
        <h2 id="project-invitations-heading" className="text-h3 text-foreground">
          Project invitations
        </h2>
      ...
      {invitationsQuery.isError && (
        <DashboardSectionError
          message={invitationsQuery.error?.message ?? "Unable to load project invitations."}
      ...
      {!invitationsQuery.isLoading && !invitationsQuery.isError && invitations.length === 0 && (
        <div className="rounded-xl border border-dashed border-border bg-surface/40 px-4 py-6 text-center">
          <p className="text-body">No pending project invitations right now.</p>
        </div>
      )}
      ...
      {!invitationsQuery.isLoading && !invitationsQuery.isError && invitations.length > 0 && (
        <div className="space-y-2">
          {invitations.map((invitation) => {
            const accepting = acceptMutation.isPending && acceptMutation.variables?._id === invitation._id;
            const rejecting = rejectMutation.isPending && rejectMutation.variables?._id === invitation._id;
