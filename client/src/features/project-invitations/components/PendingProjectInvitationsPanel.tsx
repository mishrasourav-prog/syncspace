...
import { useCancelProjectInvitationMutation } from "../hooks/useProjectInvitationMutations";

import { useProjectInvitationsQuery } from "../hooks/useProjectInvitationQueries";
...
export function PendingProjectInvitationsPanel({
  ...
  const invitationsQuery = useProjectInvitationsQuery(projectId, canView);

  const cancelMutation = useCancelProjectInvitationMutation(projectId);
  ...
  function handleCancelInvitation(invitation: ProjectInvitation): void {
    cancelMutation.mutate(invitation._id, {
      onSuccess: () => {
        toast.success(`Invitation for ${invitation.email} was cancelled.`);
      },
    ...
    >
      ...
      {invitationsQuery.isError && (
        <DashboardSectionError
          compact
          message={
            invitationsQuery.error?.message ?? "Unable to load invitations."
          }
          onRetry={() => {
      ...
      {!invitationsQuery.isLoading &&
        !invitationsQuery.isError &&
        invitations.length === 0 && (
          <p className="text-caption">No pending invitations.</p>
        )}
          ...
          <div className="space-y-2">
            {invitations.map((invitation) => {
              const isCancelling =
                ...
                cancelMutation.variables === invitation._id;
