...
export function ProjectReadOnlyBanner({ project, workspace, role }: ProjectReadOnlyBannerProps) {
  if (workspace.isArchived) {
    return (
      <div className="flex items-start gap-2.5 rounded-lg border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning">
        ...
        <p>This project's workspace is archived, so the project is read-only until the workspace is restored.</p>
      </div>
    );
  }
