...
interface ManageTaskAssigneesProps {
  ...
  assigneeDetails?: Record<string, TaskAssignee>;
}
...
export function ManageTaskAssignees({
  ...
  const search = searchState.taskId === task._id ? searchState.value : "";
  ...
  const assignMutation = useAssignTaskMemberMutation(projectId);

  const removeMutation = useRemoveTaskAssigneeMutation(projectId);

  const assignedIds = useMemo(
    () => new Set(task.assignees.map((assignee) => assignee._id)),
    [task.assignees],
  );
  ...
  const availableMembers = useMemo(() => {
    ...
    return members.filter((member) => {
      if (assignedIds.has(member.user._id)) {
        return false;
      }
  ...
  function handleAssign(userId: string): void {
    if (isMutating) {
      return;
    }
    ...
    assignMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast.success(
            member ? `${member.user.name} was assigned.` : "Member assigned.",
          );
        },

        onError: (error) => {
          toast.error(error.message ?? "Unable to assign member.");
        },
      },
    );
  }
  ...
  function handleRemove(userId: string): void {
    ...
    removeMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast.success(
            assignee ? `${assignee.name} was unassigned.` : "Assignee removed.",
          );
        },

        onError: (error) => {
          toast.error(error.message ?? "Unable to remove assignee.");
        },
      },
    );
  }
    ...
    <div>
      <div className="flex flex-wrap items-center gap-1.5">
        {task.assignees.map((assignee) => {
          const detail = assigneeDetails?.[assignee._id];
        ...
        {task.assignees.length === 0 && (
          <span className="text-caption">No one assigned yet.</span>
        )}
