...
export function ProjectCompletionPanel({
  ...
  const stats = useMemo(() => {
    ...
    const doneActive = activeItems.filter(
      (task) => task.status === "DONE",
    ).length;
    const completionPercentage =
      totalActive === 0 ? 0 : Math.round((doneActive / totalActive) * 100);
    ...
    const completedLast7Days = activeItems.filter(
      (task) =>
        task.completedAt &&
        new Date(task.completedAt).getTime() >= sevenDaysAgo,
    ).length;
  ...
  const maxDailyCount = Math.max(
    1,
    ...stats.dailyCounts.map((day) => day.count),
  );
  ...
  if (isError) {
    return (
      <div className="rounded-xl border border-border bg-surface/60 p-4 shadow-soft">
        <h3 className="mb-4 text-h3 text-foreground">Completion</h3>

        <DashboardSectionError
          compact
          message={errorMessage ?? "Unable to load completion information."}
          onRetry={onRetry}
        />
      </div>
    );
    ...
    <div className="rounded-xl border border-border bg-surface/60 p-4 shadow-soft">
      ...
      <dl className="mt-4 grid grid-cols-3 gap-2 text-center">
        ...
        <div className="rounded-lg bg-background/50 px-2 py-2">
          ...
          <dd className="mt-0.5 text-sm font-semibold text-danger">
            {stats.overdueCount}
          </dd>
        </div>
        <div className="rounded-lg bg-background/50 px-2 py-2">
      ...
      <div className="mt-4">
        ...
        <div
          ...
          role="img"
          aria-label="Completed work items over the last 7 days"
        >
          {stats.dailyCounts.map((day) => (
            ...
            >
              <div
                ...
                style={{
                  height: `${Math.max(4, (day.count / maxDailyCount) * 48)}px`,
                }}
