...
/* CSS custom properties so the donut always matches the current theme. */
...
export function WorkItemStatusOverview({  
    ...
    isError,
    errorMessage,
    onRetry, }: WorkItemStatusOverviewProps) {
  const [filter, setFilter] = useState<WorkFilter>("all");

  const activeItems = useMemo(
    () => tasks.filter((task) => !task.isArchived && (filter === "all" || task.type === filter)),
    [tasks, filter]
  );

  const counts = useMemo(() => {
    const result: Record<TaskStatus, number> = { TODO: 0, IN_PROGRESS: 0, IN_REVIEW: 0, DONE: 0 };
...
if (
    ...
    return (
        <div className="rounded-xl border border-border bg-surface/60 p-4 shadow-soft">
            ...
            <DashboardSectionError
                ...
                message={
                    errorMessage ??
                    "Unable to load work-item status."
                }
                onRetry={
                    onRetry
                }
  ...
  return (
    <div className="rounded-xl border border-border bg-surface/60 p-4 shadow-soft">
          ...
          {(["all", "task", "issue"] as const).map((option) => (
            <button
              ...
              className={cn(
                ...
                filter === option ? "bg-primary/15 text-primary" : "text-muted hover:text-foreground"
              )}
            >
              {option === "all" ? "All Work" : option === "task" ? "Tasks" : "Issues"}
            </button>
          ))}
        </div>
      </div>
      ...
      {total === 0 ? (
        <div className="flex h-40 items-center justify-center text-caption">No active work items</div>
      ) : (
        <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center sm:justify-around">
          <div className="relative h-[160px] w-[160px] shrink-0">
            <svg viewBox="0 0 160 160" className="h-full w-full -rotate-90" role="img" aria-label={`${total} active work items by status`}>
