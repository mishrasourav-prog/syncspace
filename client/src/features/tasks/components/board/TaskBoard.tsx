...
import { ALL_STATUSES } from "../../task.filters";
...
export function TaskBoard({
  ...
  function findColumnOf(taskId: string): TaskStatus | undefined {
    return ALL_STATUSES.find((status) =>
      columns[status].some((task) => task._id === taskId),
    );
  }
  ...
  function handleDragStart(event: DragStartEvent): void {
    ...
    const task = columns[column].find((item) => item._id === activeId);
  ...
  function handleDragEnd(event: DragEndEvent): void {
    ...
    const isOverColumn = ALL_STATUSES.includes(over.id as TaskStatus);
    ...
    if (sourceColumn === targetColumn) {
      const sourceList = columns[sourceColumn];

      const oldIndex = sourceList.findIndex((task) => task._id === activeId);

      const newIndex = isOverColumn
        ? sourceList.length - 1
        : sourceList.findIndex((task) => task._id === overId);

      if (oldIndex < 0 || newIndex < 0 || oldIndex === newIndex) {
        return;
      }

      next[sourceColumn] = arrayMove(sourceList, oldIndex, newIndex);

      onReorder(next, [sourceColumn]);

      return;
    }

    const sourceList = next[sourceColumn];

    const activeIndex = sourceList.findIndex((task) => task._id === activeId);
    ...
    if (!isOverColumn) {
      const overIndex = targetList.findIndex((task) => task._id === overId);
    ...
    onReorder(next, [sourceColumn, targetColumn]);
  }
