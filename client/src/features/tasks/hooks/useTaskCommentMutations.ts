...
export function useCreateTaskCommentMutation(projectId: string, taskId: string) {
  ...
  return useMutation<TaskComment, ApiErrorShape, CreateTaskCommentPayload>({
    ...
    onSuccess: (createdComment) => {
      queryClient.setQueryData<InfiniteData<TaskCommentsPage>>(queryKey, (previous) => {
        if (!previous || previous.pages.length === 0) {
          return {
            pages: [{ comments: [createdComment], nextCursor: null, hasMore: false }],
...
export function useUpdateTaskCommentMutation(projectId: string, taskId: string) {
  const queryClient = useQueryClient();
  const queryKey = taskCommentQueryKeys.list(projectId, taskId, PAGE_LIMIT);

  return useMutation<TaskComment, ApiErrorShape, { commentId: string; body: string }>({
    mutationFn: ({ commentId, body }) => updateTaskCommentRequest(commentId, { body }),
    onSuccess: (updatedComment) => {
      queryClient.setQueryData<InfiniteData<TaskCommentsPage>>(queryKey, (previous) =>
        replaceCommentInPages(previous, updatedComment)
      );
...
export function useDeleteTaskCommentMutation(projectId: string, taskId: string) {
  const queryClient = useQueryClient();
  const queryKey = taskCommentQueryKeys.list(projectId, taskId, PAGE_LIMIT);
  ...
  return useMutation<TaskComment, ApiErrorShape, string>({
    ...
    onSuccess: (deletedComment) => {
      queryClient.setQueryData<InfiniteData<TaskCommentsPage>>(queryKey, (previous) =>
        replaceCommentInPages(previous, deletedComment)
      );
