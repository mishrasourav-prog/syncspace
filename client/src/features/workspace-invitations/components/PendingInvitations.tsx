...
} from "../hooks/useWorkspaceInvitationMutations";
...
export function PendingInvitations() {
  ...
  function handleReject(invitationId: string) {
    rejectMutation.mutate(invitationId, {
      ...
      onError: (error) =>
        toast.error(error.message ?? "Unable to decline invitation."),
    });
  }
          ...
          <div className="space-y-2">
            {invitations.map((invitation) => {
              const isAccepting =
                ...
                acceptMutation.variables === invitation._id;
              const isRejecting =
                rejectMutation.isPending &&
                rejectMutation.variables === invitation._id;
