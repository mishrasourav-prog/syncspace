...
//   // Join/leave the workspace's realtime room, and refresh activity on new events.
//   useEffect(() => {
//     if (!workspaceId) return;

//     socket.emit("workspace:join", workspaceId, () => {
//       // A failed room join simply means realtime updates won't arrive;
//       // the HTTP workspace query above remains the source of truth for access.
//     });

//     function handleActivityNew(payload: { workspaceId: string; projectId: string; activityId: string }) {
...
//   useEffect(() => {
//     if (workspaceQuery.isError && workspaceQuery.error?.status === 404 && !hasShownNotFoundToast.current) {
...
// | - bookmarked URLs containing hashes
// |
// */
...
//         <p className="text-body">{workspaceQuery.error?.message ?? "Unable to load this workspace."}</p>
...
//         onArchive={() => setActionTarget({ type: "archive", workspace })}
//         onRestore={() => setActionTarget({ type: "restore", workspace })}
//         onLeave={() => setActionTarget({ type: "leave", workspace })}
//       />
...
//               <WorkspaceMembersPanel workspace={workspace} search={search} onInvite={() => setInviteOpen(true)} />
//             </div>
//           </div>
...
//           <WorkspaceActivityFeed workspaceId={workspace._id} projects={projectsQuery.data ?? []} />
//         </div>
//       </div>

//       <InviteWorkspaceMemberDialog workspace={workspace} open={inviteOpen} onClose={() => setInviteOpen(false)} />
...
} from "@/features/activity/components/WorkspaceActivityFeed";
...
} from "@/features/projects/components/WorkspaceProjectsPanel";

import {
    useWorkspaceMembersQuery,
} from "@/features/workspace-members/hooks/useWorkspaceMemberQueries";

import {
    WorkspaceMembersPanel,
} from "@/features/workspace-members/components/WorkspaceMembersPanel";

import {
    InviteWorkspaceMemberDialog,
} from "@/features/workspace-members/components/InviteWorkspaceMemberDialog";
...
import {
    EditWorkspaceDialog,
} from "../components/EditWorkspaceDialog";
...
import {
    WorkspaceHeader,
} from "../components/overview/WorkspaceHeader";

import {
    WorkspaceReadOnlyBanner,
} from "../components/overview/WorkspaceReadOnlyBanner";

import {
    WorkspaceOverviewNavigation,
} from "../components/overview/WorkspaceOverviewNavigation";

import {
    WorkspaceOverviewMetrics,
} from "../components/overview/WorkspaceOverviewMetrics";

import {
    WorkspaceAccessPanel,
} from "../components/overview/WorkspaceAccessPanel";
...
export function WorkspaceOverviewPage() {
    ...
    const [
        ...
        useState<WorkspaceActionTarget | null>(
            null
        );

    const hasShownNotFoundToast =
        useRef(
            false
        );
    ...
    /*
    |--------------------------------------------------------------------------
    | Reset Not-Found Toast Guard
    |--------------------------------------------------------------------------
    |
    | This component can remain mounted while the workspace ID changes.
    | Resetting the guard allows an inaccessible new workspace to show
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Workspace Room and Activity Events
    |--------------------------------------------------------------------------
    */
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Handles sidebar navigation, mobile navigation, browser history,
    | and bookmarked URLs containing workspace section hashes.
    |
    */
