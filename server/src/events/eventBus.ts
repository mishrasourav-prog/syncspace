...
export class EventBus {
  ...
  subscribe<TName extends keyof DomainEventPayloadMap>(
    eventName: TName,
    handler: DomainEventHandler<TName>,
  ): () => void {
    const currentHandlers =
      this.handlers.get(eventName) ?? new Set<AnyEventHandler>();

    currentHandlers.add(handler as unknown as AnyEventHandler);
    ...
    return () => {
      ...
      registeredHandlers.delete(handler as unknown as AnyEventHandler);
  ...
  async publish<TName extends keyof DomainEventPayloadMap>(
    eventName: TName,
    ...
    const results = await Promise.allSettled(
      Array.from(registeredHandlers).map(async (handler) => {
    ...
    for (const result of results) {
      if (result.status === "rejected") {
        console.error(
          `Domain event handler failed for "${String(eventName)}".`,
          result.reason,
        );
      }
    }
