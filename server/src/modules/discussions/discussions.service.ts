...
import Discussion, { type IDiscussion } from "./discussions.model";
...
class DiscussionService {
  ...
  private async getDiscussionContext(
    ...
    const discussion = await Discussion.findById(discussionId)
      .select("_id workspace project title author isPinned isLocked isDeleted")
  ...
  private async getReplyContext(replyId: string): Promise<IReplyContext> {
    const reply = await DiscussionReply.findById(replyId)
      .select("_id project discussion author isDeleted")
      .lean<IReplyContext>();
  ...
  private async getDiscussionForResponse(
    ...
    return mapDiscussion({
      ...(discussion as unknown as IDiscussionForResponse),
  ...
  private async getReplyForResponse(
    ...
    return mapDiscussionReply(reply as unknown as IDiscussionReplyForResponse);
  }
  ...
  async createDiscussion(
    ...
    const { project } = await this.getProjectContext(projectId, userId, true);
    ...
    await eventBus.publish(DomainEventName.DISCUSSION_CREATED, {
    ...
    return this.getDiscussionForResponse(discussion._id);
  }
  ...
  async getProjectDiscussions(
    ...
    await this.getProjectContext(projectId, userId, false);

    const filter: FilterQuery<IDiscussion> = {
    ...
    const hasMore = discussions.length > query.limit;

    const selected = hasMore ? discussions.slice(0, query.limit) : discussions;

    const discussionIds = selected.map((discussion) => discussion._id);
    ...
    return {
      discussions: selected.map((discussion) =>
        mapDiscussion({
          ...(discussion as unknown as IDiscussionForResponse),

          replyCount: replyCountMap.get(discussion._id.toString()) ?? 0,
        }),
      ),

      nextCursor: hasMore
        ? (selected[selected.length - 1]?._id.toString() ?? null)
        : null,
    };
  }
  ...
  async getDiscussionById(
    ...
    await this.getProjectContext(discussion.project.toString(), userId, false);

    return this.getDiscussionForResponse(discussionId);
  }
  ...
  async updateDiscussion(
    ...
    await this.getProjectContext(discussion.project.toString(), userId, true);
    ...
    if (discussion.isLocked) {
      throw new ApiError(409, "Locked discussions cannot be edited.");
    }
    ...
    const updatedDiscussion = await Discussion.findOneAndUpdate(
      ...
      .select("_id workspace project title")
      .lean<{
        _id: Types.ObjectId;
    ...
    if (!updatedDiscussion) {
      throw new ApiError(409, "Discussion could not be updated.");
    }

    await eventBus.publish(DomainEventName.DISCUSSION_UPDATED, {
      workspaceId: updatedDiscussion.workspace.toString(),

      projectId: updatedDiscussion.project.toString(),
  ...
  async deleteDiscussion(discussionId: string, userId: string): Promise<void> {
    ...
    const { membership } = await this.getProjectContext(
    ...
    if (result.modifiedCount === 0) {
      throw new ApiError(409, "Discussion could not be deleted.");
    }

    await eventBus.publish(DomainEventName.DISCUSSION_DELETED, {
      workspaceId: discussion.workspace.toString(),

      projectId: discussion.project.toString(),
  ...
  private async updateModerationState(
    ...
    await eventBus.publish(eventName, {
      ...
      discussionId: updated._id.toString(),
  ...
  async createReply(
    ...
    if (discussion.isLocked) {
      throw new ApiError(409, "This discussion is locked.");
    }
    ...
    await eventBus.publish(DomainEventName.DISCUSSION_REPLY_CREATED, {
      workspaceId: discussion.workspace.toString(),
      ...
      discussionAuthorId: discussion.author.toString(),

      title: discussion.title,
    });

    return this.getReplyForResponse(reply._id);
  }

  async getReplies(
    discussionId: string,
    userId: string,
    query: IDiscussionReplyListQuery,
    ...
    const hasMore = replies.length > query.limit;

    const selected = hasMore ? replies.slice(0, query.limit) : replies;

    return {
      replies: selected.map((reply) =>
        mapDiscussionReply(reply as unknown as IDiscussionReplyForResponse),
      ),
  ...
  async updateReply(
    replyId: string,
    userId: string,
    data: IUpdateDiscussionReplyInput,
  ): Promise<IDiscussionReplyResponse> {
    const reply = await this.getReplyContext(replyId);

    const discussion = await this.getDiscussionContext(
      reply.discussion.toString(),
    );

    await this.getProjectContext(reply.project.toString(), userId, true);

    if (reply.author.toString() !== userId) {
      throw new ApiError(403, "Only the reply author can edit this reply.");
    }
    ...
    if (!updated) {
      throw new ApiError(409, "Reply could not be updated.");
    }

    await eventBus.publish(DomainEventName.DISCUSSION_REPLY_UPDATED, {
      workspaceId: discussion.workspace.toString(),
    ...
    return this.getReplyForResponse(replyId);
  }
  ...
  async deleteReply(replyId: string, userId: string): Promise<void> {
    ...
    const result = await DiscussionReply.updateOne(
      {
        _id: reply._id,
    ...
    if (result.modifiedCount === 0) {
      throw new ApiError(409, "Reply could not be deleted.");
    }

    await eventBus.publish(DomainEventName.DISCUSSION_REPLY_DELETED, {
      workspaceId: discussion.workspace.toString(),
