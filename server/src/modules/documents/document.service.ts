...
import ProjectDocument, { type IProjectDocument } from "./document.model";
...
class DocumentService {
  ...
  private async getDocumentContext(
    ...
    const document = await ProjectDocument.findById(documentId)
      .select("_id workspace project title createdBy revision isArchived")
  ...
  private async getDocumentForResponse(
    ...
    const document = await ProjectDocument.findById(documentId)
      ...
      .populate("updatedBy", "name username avatar")
      .lean()
      .exec();
    ...
    return mapProjectDocument(
      document as unknown as IProjectDocumentForResponse,
    );
  }
  ...
  async createDocument(
    ...
    await eventBus.publish(DomainEventName.DOCUMENT_CREATED, {
    ...
    return this.getDocumentForResponse(document._id);
  }

  async getProjectDocuments(
    projectId: string,
    userId: string,
    query: IProjectDocumentListQuery,
    ...
    const selectedDocuments = hasMore
      ? documents.slice(0, query.limit)
      : documents;

    const nextCursor = hasMore
      ? (selectedDocuments[selectedDocuments.length - 1]?._id.toString() ??
        null)
      : null;

    return {
      documents: selectedDocuments.map((document) =>
        mapProjectDocument(document as unknown as IProjectDocumentForResponse),
      ),
  ...
  async getDocumentById(
    ...
    await this.getProjectContext(document.project.toString(), userId, false);

    return this.getDocumentForResponse(documentId);
  }
  ...
  async updateDocument(
    ...
    const existingDocument = await this.getDocumentContext(documentId);

    await this.getProjectContext(
      existingDocument.project.toString(),
      userId,
      true,
    );

    if (existingDocument.isArchived) {
      throw new ApiError(409, "Archived documents are read-only.");
    }

    if (existingDocument.revision !== data.expectedRevision) {
      throw new ApiError(
        409,
    ...
    const updatedDocument = await ProjectDocument.findOneAndUpdate(
      {
      ...
      .select("_id workspace project title revision")
      .lean<{
        _id: Types.ObjectId;
    ...
    await eventBus.publish(DomainEventName.DOCUMENT_UPDATED, {
      workspaceId: updatedDocument.workspace.toString(),

      projectId: updatedDocument.project.toString(),
  ...
  async archiveDocument(
    ...
    if (document.isArchived) {
      throw new ApiError(409, "Document is already archived.");
    }

    const archivedDocument = await ProjectDocument.findOneAndUpdate(
      {
        _id: document._id,
    ...
    await eventBus.publish(DomainEventName.DOCUMENT_ARCHIVED, {
      workspaceId: archivedDocument.workspace.toString(),

      projectId: archivedDocument.project.toString(),

      documentId: archivedDocument._id.toString(),
  ...
  async restoreDocument(
    ...
    if (!document.isArchived) {
      throw new ApiError(409, "Document is not archived.");
    }

    const restoredDocument = await ProjectDocument.findOneAndUpdate(
      {
        _id: document._id,
    ...
    await eventBus.publish(DomainEventName.DOCUMENT_RESTORED, {
      workspaceId: restoredDocument.workspace.toString(),

      projectId: restoredDocument.project.toString(),

      documentId: restoredDocument._id.toString(),
