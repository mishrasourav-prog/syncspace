...
interface INotificationForResponse {
  ...
  actor: IPopulatedNotificationActor | null;
...
export class NotificationService {
  private mapNotification(
    notification: INotificationForResponse,
  ): INotificationResponse {
    const actor: INotificationActor | null = notification.actor
      ? {
    ...
    return {
      ...
      workspace: notification.workspace?.toString() ?? null,

      project: notification.project?.toString() ?? null,
  ...
  async createNotification(data: CreateNotificationInput): Promise<string> {
    const notification = await Notification.create({
      recipient: new Types.ObjectId(data.recipientId),

      actor: data.actorId ? new Types.ObjectId(data.actorId) : undefined,
  ...
  async createAndPublishNotification(
    ...
    await eventBus.publish(DomainEventName.NOTIFICATION_CREATED, {
      notificationId,
  ...
  async markAsRead(
    ...
    return this.mapNotification(populatedNotification);
  }
