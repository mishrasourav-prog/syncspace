...
export const inviteProjectMember = async (
  ...
  try {
    ...
    const data = createProjectInvitationSchema.parse(req.body);

    const invitation = await ProjectInvitationService.inviteMember(
      projectId,
      req.user._id,
      data,
    );
...
export const acceptProjectInvitation = async (
  ...
  try {
    ...
    const { invitationId } = projectInvitationIdSchema.parse(req.params);

    await ProjectInvitationService.acceptInvitation(invitationId, req.user._id);
...
export const rejectProjectInvitation = async (
  ...
  try {
    ...
    await ProjectInvitationService.rejectInvitation(invitationId, req.user._id);

    return res
      .status(200)
      .json(new ApiResponse(200, "Project invitation rejected successfully."));
  } catch (error) {
    return next(error);
  }
};
...
export const cancelProjectInvitation = async (
  ...
  try {
    ...
    await ProjectInvitationService.cancelInvitation(invitationId, req.user._id);

    return res
      .status(200)
      .json(new ApiResponse(200, "Project invitation cancelled successfully."));
  } catch (error) {
    return next(error);
  }
};
...
export const getPendingProjectInvitations = async (
  ...
  try {
    ...
    const payload = await ProjectInvitationService.getPendingInvitations(
      projectId,
      req.user._id,
    );
