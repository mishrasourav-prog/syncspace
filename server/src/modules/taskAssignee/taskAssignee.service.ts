...
import Task, { ITaskDocument } from "../tasks/task.model";

import Project, { IProjectDocument } from "../project/project.model";

import ProjectMember from "../projectMember/projectMember.model";

import { Workspace, IWorkspaceDocument } from "../workspace/workspace.model";
...
export class TaskAssigneeService {
  ...
  private assertAssignmentsAreWritable(context: ITaskAssignmentContext): void {
    if (context.workspace.isArchived) {
      throw new ApiError(
        409,
  ...
  async assignMember(
    ...
    const context = await this.getTaskContextForMember(taskId, userId);

    this.assertAssignmentsAreWritable(context);

    const [assigneeProjectMembership, assigneeWorkspaceMembership] =
      await Promise.all([
        ProjectMember.exists({
    ...
    if (!assigneeProjectMembership || !assigneeWorkspaceMembership) {
      throw new ApiError(
        409,
    ...
    const existingAssignment = await TaskAssignee.exists({
    ...
    const assignment = await TaskAssignee.create({
      task: context.task._id,

      user: new Types.ObjectId(assigneeId),
    ...
    if (!populatedAssignment.user) {
      throw new ApiError(409, "The assigned user is no longer available.");
    }
    ...
    return this.mapAssignment(populatedAssignment, populatedAssignment.user);
  }
  ...
  async getTaskAssignees(
    ...
    await this.getTaskContextForMember(taskId, userId);
    ...
    const assignees: ITaskAssigneeResponse[] = [];
    ...
    for (const assignment of assignments) {
      ...
      assignees.push(this.mapAssignment(assignment, assignment.user));
    }
  ...
  async removeAssignee(
    ...
    if (!assignment) {
      throw new ApiError(404, "User is not assigned to this task.");
    }

    await assignment.deleteOne();

    await eventBus.publish(DomainEventName.TASK_UNASSIGNED, {
      workspaceId: context.workspace._id.toString(),
      projectId: context.project._id.toString(),
      taskId: context.task._id.toString(),
      actorId: userId,
