...
import {
  WorkspaceInvitation,
} from "../workspaceInvitation/workspaceInvitation.model";
...
export interface ProfileMutationResult {
  ...
  /**
   * The controller may remove this old hosted asset after the database update
   * succeeds. It remains null when the mutation did not replace an avatar.
   */
...
export interface DeletedAccountCleanup {
  /**
   * Returned only to the controller so external avatar storage can be cleaned
   * after the MongoDB transaction commits.
   */
  avatarPublicId:
    string |
    null;
...
export class UserService {
  ...
  */
  ...
  private memberProfileNotFound(): ApiError {
    /*
    Always use one privacy-safe response. The caller must not learn whether
    the user, workspace, project, or membership exists.
    */
    return new ApiError(
      404,
  ...
  */
  ...
  async updateSelfProfile(
    ...
    if (
      ...
      /*
      A direct hosted URL is not owned by the previous Cloudinary record.
      Clear the stored public ID so future deletion never targets the wrong
      asset.
      */
      user.avatarPublicId =
        null;
  ...
  */
  ...
  private async buildDeletionReadiness(
    ...
    if (
      ...
      ownedWorkspacesQuery.session(
        session
      );

      adminMembershipsQuery.session(
        session
      );
    }
    ...
    let lastAdminProjects:
      AccountDeletionReadiness["blockers"]["lastAdminProjects"] =
        [];
    ...
    if (
      ...
      const adminCountAggregation =
        ProjectMember.aggregate<ProjectAdminCount>([
          {
            $match: {
              project: {
                $in:
      ...
      if (
        ...
        adminCountAggregation.session(
          session
        );
      }

      const adminCounts =
        await adminCountAggregation.exec();

      const adminCountByProject =
        new Map(
          adminCounts.map(
            (
              result
      ...
      const lastAdminProjectIds =
        candidateProjectIds.filter(
          (
            projectId
          ) =>
            (
              adminCountByProject.get(
                projectId.toString()
              ) ??
              0
            ) <=
    ...
    const normalizedOwnedWorkspaces =
      ownedWorkspaces.map(
        (
          workspace
        ) => ({
    ...
    return {
      canDelete:
        normalizedOwnedWorkspaces.length ===
          0 &&
        lastAdminProjects.length ===
          0,

      blockers: {
        ownedWorkspaces:
          normalizedOwnedWorkspaces,
  ...
  async getDeletionReadiness(
    ...
    return this.buildDeletionReadiness(
      objectId
    );
  }
    ...
    try {
      await session.withTransaction(
        async () => {
          ...
          const readiness =
            await this.buildDeletionReadiness(
              user._id,
              session
            );
          ...
          /*
          Remove private operational records. Historical authorship references
          in tasks, documents, comments, discussions, and activity are kept and
          will resolve to this anonymized User tombstone.
          */
          await Promise.all([
            ...
            await User.updateOne(
              ...
              },
              {
                $set: {
                  ...
                  username:
                    `Deleted${tombstoneSuffix}`,

                  email:
                    `deleted.${tombstoneSuffix}@syncspace.invalid`,
  ...
  */
  ...
  async getMemberProfile(
    ...
    let workspaceContext:
      MemberProfile["context"]["workspace"] =
        null;

    let projectContext:
      MemberProfile["context"]["project"] =
        null;
    ...
    /*
    Query the target account only after context authorization so an outsider
    cannot use this endpoint to probe account existence.
    */
    const targetUser =
      await User.findOne({
        _id:
    ...
    return {
      ...
      createdAt:
        targetUser.createdAt.toISOString(),
