...
/*
|--------------------------------------------------------------------------
| Workspace Avatar Authorization
|--------------------------------------------------------------------------
|
| This runs before Multer so unauthorized members cannot make the server
| process an avatar file. The service checks the permission again when the
| final database update occurs, protecting against role/archive races.
|
*/

export const authorizeWorkspaceAvatarManagement =
  async (
    req: Request,
    _res: Response,
    next: NextFunction
...
export const replaceWorkspaceAvatar =
  async (
    req: Request,
    res: Response,
    next: NextFunction
    ...
    try {
      ...
      const uploadedAvatar =
        await uploadWorkspaceAvatarBuffer(
          req.file.buffer,
          workspaceId
        );
      ...
      let result:
        Awaited<
          ReturnType<
            typeof WorkspaceService.replaceWorkspaceAvatar
          >
        >;

      try {
        result =
          await WorkspaceService.replaceWorkspaceAvatar(
            workspaceId,
            getAuthenticatedUserId(
              req
            ),
        ...
        await deleteWorkspaceAvatarAssetBestEffort(
          uploadedAvatar.publicId
        );
...
export const removeWorkspaceAvatar =
  async (
    req: Request,
    res: Response,
    next: NextFunction
    ...
    try {
      ...
      const result =
        await WorkspaceService.removeWorkspaceAvatar(
          workspaceId,
          getAuthenticatedUserId(
            req
          )
      ...
      return res.status(200).json(
        new ApiResponse(
          200,
          "Workspace avatar removed successfully.",
          {
            workspace:
              result.workspace,
          }
