...
export const inviteUser = async (
  ...
  try {
    ...
    const { workspaceId } = workspaceInvitationWorkspaceParamsSchema.parse(
      req.params,
    );

    const data = inviteUserSchema.parse(req.body);

    const invitation = await WorkspaceInvitationService.inviteUser(
      workspaceId,
      req.user._id,
      data,
    );
    ...
    return res
      .status(201)
      .json(new ApiResponse(201, "Invitation sent successfully.", invitation));
  } catch (error) {
    return next(error);
  }
};
